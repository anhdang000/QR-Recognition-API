...
classes = open('yolov3/qrcode.names').read().strip().split('\n')
net = cv.dnn.readNetFromDarknet('yolov3/qrcode-yolov3-tiny.cfg', 'yolov3/qrcode-yolov3-tiny.weights')
...
def postprocess(frame, outs):
    ...
    for i in indices:
        ...
        width = box[2]
        height = box[3]
        print('\t|\t'.join(['x_min', 'y_min', 'x_max', 'y_max']))
        print('\t|\t'.join([str(left), str(top), str(left + width), str(top + height)]))
